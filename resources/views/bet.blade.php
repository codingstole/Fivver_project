<!DOCTYPE html><!--[if lt IE 7]>      <html class="no-js lt-ie9 lt-ie8 lt-ie7" lang="en-US"> <![endif]--><!--[if IE 7]>         <html class="no-js lt-ie9 lt-ie8" lang="en-US"> <![endif]--><!--[if IE 8]>         <html class="no-js lt-ie9" lang="en-US"> <![endif]--><!--[if gt IE 8]><!--><html class="no-js" lang="en-US"><!--<![endif]--><head>
  ...
  <title>Bet – Exchange Alternative – Cryptocurrency</title>
...
<body>
  <h1>Cryptocurrency Price Betting</h1>


  <div class="leaderboard">
    <h2>Leaderboard</h2>
    <ol>
      <li><span id="leaderboard-1"></span> 
        <span id="leaderboard-1-points">5_stars</span> points</li>
      <li><span id="leaderboard-2"></span>
         <span id="leaderboard-2-points">3_stars</span> points</li>
      <li><span id="leaderboard-3"></span>
          <span id="leaderboard-3-points">1_stars</span> points</li>
    </ol>
  </div>

  <div class="betting">
    <h2>Betting</h2>
    <form id="betting-form"  action="{{route('registration.post')}}" method="POST">
      @csrf
...
<script >
    // Get the current prices of the tracked cryptocurrencies.
function getPrices() {
  var request = new XMLHttpRequest();
  request.open("GET", "https://api.coingecko.com/api/v3/simple/price?ids=BTC,ETH,BNB,MATIC,SOL,DOGE&vs_currencies=usd", true);
  request.onload = function() {
    if (request.status >= 200 && request.status < 400) {
      var data = JSON.parse(request.responseText);

      // Update the prices on the page.
      document.getElementById("btc-price").innerHTML = data.BTC.usd;
      document.getElementById("eth-price").innerHTML = data.ETH.usd;
      document.getElementById("bnb-price").innerHTML = data.BNB.usd;
      document.getElementById("matic-price").innerHTML = data.MATIC.usd;
      document.getElementById("sol-price").innerHTML = data.SOL.usd;
      document.getElementById("doge-price").innerHTML = data.DOGE.usd;
    } else {
      console.error("Error getting prices:", request.statusText);
    }
  };
...
function getLeaderboard() {
  ...
  request.onload = function() {
    if (request.status >= 200 && request.status < 400) {
      ...
      document.getElementById("leaderboard-1").innerHTML = data[0].name;
      document.getElementById("leaderboard-1-points").innerHTML = data[0].points;
      document.getElementById("leaderboard-2").innerHTML = data[1].name;
      document.getElementById("leaderboard-2-points").innerHTML = data[1].points;
      document.getElementById("leaderboard-3").innerHTML = data[2].name;
      document.getElementById("leaderboard-3-points").innerHTML = data[2].points;
    } else {
      console.error("Error getting leaderboard:", request.statusText);
    }
  };
  request.onerror = function() {
    console.error("Error getting leaderboard:", request.statusText);
...
// Place a bet.
function placeBet(crypto, price, time) {
  var request = new XMLHttpRequest();
  request.open("POST", "bet.php", true);
  request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  request.onload = function() {
    if (request.status >= 200 && request.status < 400) {
      // Bet was placed successfully.

      // Update the leaderboard.
      getLeaderboard();
    } else {
      console.error("Error placing bet:", request.statusText);
    }
  };
  request.onerror = function() {
    console.error("Error placing bet:", request.statusText);
  };
  request.send("crypto=" + crypto + "&price=" + price + "&time=" + time);
}
